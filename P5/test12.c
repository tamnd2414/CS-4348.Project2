...
int main(int argc, char *argv[]){
  ...
  for(int i = ROOTINO; i < sbPtr->ninodes; i++){//scan through all the inodes
    int refcount = 0;
    ...
    for(int j = ROOTINO; j < sbPtr->ninodes; j++){//find directory and scan through all the entries
      if(dinodePtr[j].type == 0 || j == i)
        continue;
      if(dinodePtr[j].type != T_DIR)
        continue;

      int n = dinodePtr[j].size/sizeof(struct dirent);
      struct dirent *de;
      de = (struct dirent *)(addr + (dinodePtr[j].addrs[0])*BLOCK_SIZE);
      for(int k = 0; k < n; k++, de++){//scan through the entries
